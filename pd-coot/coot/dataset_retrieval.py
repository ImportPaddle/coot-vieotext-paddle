...
class RetrievalDataPointTuple(typext.TypedNamedTuple):
    ...
    data_key: str
    sentences: List[str]
    vid_feat: paddle.Tensor  # shape (num_feat, vid_feat_dim)
    vid_feat_len: int
    par_feat: paddle.Tensor  # shape (num_tokens, text_feat_dim)
    par_feat_len: int
    clip_num: int
    clip_feat_list: List[paddle.Tensor]  # shapes (num_feat_clip, vid_feat_dim)
    clip_feat_len_list: List[int]
    sent_num: int
    sent_feat_list: List[paddle.Tensor]  # shapes (num_tokens_sent, text_feat_dim)
...
class RetrievalDataBatchTuple(typext.TypedNamedTuple):
    ...
    vid_feat: paddle.Tensor  # shape (batch_size, max_num_feat, vid_feat_dim) dtype float
    vid_feat_mask: paddle.Tensor  # shape (batch_size, max_num_feat) dtype bool
    vid_feat_len: paddle.Tensor  # shape (batch_size) dtype long
    par_feat: paddle.Tensor  # shape (batch_size, max_num_tokens, text_feat_dim) dtype float
    par_feat_mask: paddle.Tensor  # shape (batch_size, max_num_tokens) dtype bool
    par_feat_len: paddle.Tensor  # shape (batch_size) dtype long
    clip_num: paddle.Tensor  # shape (batch_size) dtype long
    clip_feat: paddle.Tensor  # shapes (total_num_clips, max_num_feat_clip, vid_feat_dim) dtype float
    clip_feat_mask: paddle.Tensor  # shapes (total_num_clips, max_num_feat_clip) dtype bool
    clip_feat_len: paddle.Tensor  # shapes (total_num_clips) dtype long
    sent_num: paddle.Tensor  # shape (batch_size) dtype long
    sent_feat: paddle.Tensor  # shapes (total_num_sents, max_num_feat_sent, text_feat_dim) dtype float
    sent_feat_mask: paddle.Tensor  # shapes (total_num_sents, max_num_feat_sent) dtype bool
    sent_feat_len: paddle.Tensor  # shapes (total_num_sents) dtype long
...
class RetrievalDataset(Dataset):
    ...
    Args:
        ...
        path_data: Dataset base papaddle.
    ...
    def __getitem__(self, item: int) -> Tuple[
        ...
        if self.cfg.frames_noise != 0:
            ...
            vid_frames_noise = utils_paddle.get_truncnorm_tensor(vid_feat.shape, std=self.cfg.frames_noise)
    ...
    def collate_fn(self, data_batch: List[RetrievalDataPointTuple]):
        """
        Collate the single datapoints above. Custom collation needed since sequences have different lengpaddle.
...
def create_retrieval_datasets_and_loaders(cfg: coot.configs_retrieval.RetrievalConfig, path_data: Union[str, Path]) -> (
        Tuple[RetrievalDataset, RetrievalDataset, DataLoader, DataLoader]):
    """
...
def run_retrieval_dataset_test(train_set: RetrievalDataset, train_loader: DataLoader) -> None:
    """
