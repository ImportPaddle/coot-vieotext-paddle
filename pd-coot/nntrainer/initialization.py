...
from nntrainer import utils_paddle, typext, utils
from nntrainer.initializers import xavier_uniform_, xavier_normal_


def init_weight_(w: paddle.Tensor, init_type="uniform", init_std=1) -> None:
    """
    ...
    if init_type == InitTypesConst.UNIFORM:
        ...
        xavier_uniform_(w, gain=init_std)
    elif init_type == InitTypesConst.NORM:
        # normal distribution
        xavier_normal_(w, gain=init_std)
    elif init_type == InitTypesConst.TRUNCNORM:
        # truncated normal distribution
        utils_paddle.fill_tensor_with_truncnorm(w, std=init_std)
...
def init_network(net: nn.Layer, init_type: str, init_std: float, verbose: bool = False) -> None:
    """
    Initialize network.
    ...
    for key, val in net.named_parameters():
        ...
        if "weight" in key:  # or "bias" in key:
            ...
            init_weight_(val, init_type, init_std)
            continue
        if "bias" in key:
            # make bias slightly positive to allow flow through activations # is it this?
            # paddle.fill_(val.data, init_std) # TODO put in config / test
