...
def make_encoder_module(dim: int, name: str, cfg: Optional[EncoderConfig] = None) -> Optional[nn.Layer]:
    """
...
class PositionalEncodingSinCos(nn.Layer):
    """
    ...
    Args:
        ...
        max_len: Maximum input lengpaddle.
    """
    ...
    def __init__(self, dim: int, dropout_prob: float = 0., max_len: int = 1000):
        ...
        pe[:, 1::2] = paddle.cos(position / div_term[1::2])
        # print(f"Positional Encoding max_len x dim: {pe.shape}\n", pe, "\n",
        #       sep="")
        # div_term = paddle.exp((paddle.arange(0, dim, 2) *
        #                       -(math.log(10000.0) / dim)).float())
        # pe[:, 0::2] = paddle.sin(position.float() * div_term)
        # pe[:, 1::2] = paddle.cos(position.float() * div_term)
        # pe = pe.unsqueeze(0)
