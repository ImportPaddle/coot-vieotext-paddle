...
class MultiGenPool(nn.Layer):
    ...
    def forward(self, features, mask, lengths):
        ...
        pooled = paddle.concat(feature_stack, axis=-1)
        return pooled


class GenPool(nn.Layer):
    """
    ...
    def __init__(
        ...
        w1_head = paddle.zeros([n_heads, d_input, self.d_head])
        b1_head = paddle.zeros([n_heads, self.d_head])
        w2_head = paddle.zeros([n_heads, self.d_head, self.d_head_output])
        b2_head = paddle.zeros([n_heads, self.d_head_output])

        self.genpool_w1_head = w1_head
        self.genpool_b1_head = b1_head
        self.genpool_w2_head = w2_head
        self.genpool_b2_head = b2_head
        ...
        self.genpool_one = paddle.ones([1])
    ...
    def forward(self, features: paddle.float64, mask: paddle.bool, _lengths: paddle.int64):
        ...
        b1 = paddle.matmul(features.unsqueeze(1), self.genpool_w1_head.unsqueeze(0))
        ...
        b1 = paddle.matmul(b1, self.genpool_w2_head.unsqueeze(0))
...
class TemporalMaxPool(nn.Layer):
    def forward(self, features, mask, _lengths):
        ...
        try:
            ...
            print(
                ...
                f"{features.shape}")
            raise e
        result2, _ = paddle.max(feat_fill, axis=1)
        # (batch, feat_dim)

        return result2


class TemporalAvgPool(nn.Layer):
...
class TemporalAvgPoolFixed(nn.Layer):
    def forward(self, features, mask, lengths):
        # features (batch, seq_len, feat_dim)
        # mask (batch, seq_len) - 1 for values, 0 for padding
        # lengths (batch)
...
class TemporalLastPool(nn.Layer):
    """
    ...
    def forward(self, features, _mask, lengths):
        ...
        result2 = paddle.gather(features, 1, idx_last)
        return result2


class TemporalFirstPool(nn.Layer):
    """
