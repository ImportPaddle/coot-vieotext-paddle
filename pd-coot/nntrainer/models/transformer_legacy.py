...
class TransformerLegacy(nn.Layer):
    """
    ...
    def forward(self, features: paddle.float64, mask: paddle.bool, lengths: paddle.int64,
        ...
        if add_after_pool is not None:
            ...
            pooled = paddle.concat([pooled, add_after_pool], axis=-1)
...
class LearnableClsToken(nn.Layer):
    """
    ...
    def __init__(self, d_model: int):
        ...
        self.fixed_ones = paddle.ParamAttr(paddle.ones(1))
...
class Sublayer(nn.Layer):
    ...
    def forward(self, *args):
        ...
        if isinstance(sublayer_return, paddle.Tensor):
...
class MultiHeadAttention(nn.Layer):
    ...
    def forward(self, query, key, value, mask_expanded: Optional[any] = None, _layer_cache=None):
        """
        ...
        context_heads = paddle.matmul(attention_dropped, value_heads)
