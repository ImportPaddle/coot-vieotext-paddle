...
class BaseModelManager:
    def __init__(self, cfg: nntrainer.trainer_configs.DefaultExperimentConfig):
        """
        Class to hold all models. This is not a nn.Layer

        Args:
            cfg: Experiment config.
        """
        # this dict contains all the models
        self.model_dict: Dict[str, nn.Layer] = {}
        self.was_loaded: bool = False
        self.cfg: nntrainer.trainer_configs.DefaultExperimentConfig = cfg
        self.is_train = True

    def is_auto_cast_enabled(self) -> bool:
        """
        Given train or val state and config, determine whether auto_cast should be enabled.
    ...
    def get_model_state(self) -> Dict[str, Dict[str, paddle.Tensor]]:
        """
    ...
    def set_model_state(self, state: Dict[str, Dict[str, paddle.Tensor]]) -> None:
        """
        ...
        if isinstance(state, list):
            for i, model_name in enumerate(self.model_dict.keys()):
                ...
                self.model_dict[model_name].set_state_dict(new_state)
            return
        ...
        Returns:
            Tuple of:
                ...
                List of:
                    parameters
        """
        params_dict: Dict[str, paddle.Tensor] = dict(model.named_parameters())
        params, param_names, params_flat = [], [], []
