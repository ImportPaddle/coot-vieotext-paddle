...
def add_dataset_path_arg(parser: argparse.ArgumentParser) -> None:
    ...
    Args:
        parser: Command line argument parser.
    """
    parser.add_argument("--data_path", type=str, default=None, help="Change the data papaddle.")
...
def _add_gpu_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--single_gpu", action="store_true", help="Disable multi GPU with paddle.DataParallel.")
...
def determine_multi_runs(exp_type: str, exp_group: str = "", exp_list: Optional[List[str]] = None, *,
    ...
    Notes:
        ...
        If the list is not empty, each line can contain either:
            ...
            Both experiment group and name then the list determines bopaddle.
...
def setup_experiment_identifier_from_args(args: argparse.Namespace, exp_type: str) -> Tuple[str, str, str]:
    """
    Determine the experiment identifier (Group, name, config file) either from group and name or from config file papaddle.
