...
def get():
    ...
    # model_ext = NetworkInNetwork.create_model(opt)  ##out 128,4
    state = torch.load(
        '/home/lc/pd-FeatureLearningRotNet/pytorchRotNet/experiments/CIFAR10_RotNet_NIN4blocks/model_net_epoch200')
    torch.save(state['network'], './model_ext_pytorch.pth')
    # torch.save(model_ext.state_dict(), './model_ext_pytorch.pth')
