...
def main():
    ...
    # ----------------------------------------数据----------------------------------------
    np.random.seed(22)
    data_ext_1 = np.random.rand(1024, 3, 32, 32).astype(np.float32)
    data_ext_2 = np.random.rand(1024, 3, 32, 32).astype(np.float32)

    target_cla_1 = np.random.randint(0, 10, (1024, 1))
    target_cla_2 = np.random.randint(0, 10, (1024, 1))
    ...
    optim_ext_paddle = paddle.optimizer.Momentum(parameters=model_ext_paddle.parameters(),
                                                 ...
                                                 weight_decay=optim_params['weight_decay'])
    optim_cla_torch = torch.optim.SGD(params=model_ext_pytorch.parameters(), lr=optim_params['lr'],
                                      momentum=optim_params['momentum'],
                                      nesterov=optim_params['nesterov'] if ('nesterov' in optim_params) else False,
                                      weight_decay=optim_params['weight_decay'])
